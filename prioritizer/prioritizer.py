"""
Run this script to prioritize the actions for a given city.

To run the script, use the following command from root directory:
python -m prioritizer.prioritizer
...
ACTION_PAIRS_FILE_PATH = (
    ...
    / "data/expert_comparisons/comparisons_Martha_Dillon_C40.json"
)
...
def get_openrouter_completion(
    model_name: str, prompt_messages: list, headers: dict | None = None
) -> dict:
    """
...
def load_all_actions_data() -> dict:
    ...
    actions_dict = {
        action["ActionID"]: action for action in actions_list if action.get("ActionID")
    }

    if not actions_dict:
        print(
            "Warning: No actions loaded or actions are missing ActionID after processing."
        )
    else:
...
def load_action_pairs_from_file(file_path: Path) -> list:
    ...
    if not file_path.exists():
        print(
            f"Warning: Action pairs file not found at {file_path}. Returning empty list."
        )
        return []
    try:
        ...
        for pair in data:
            ...
            if not all(mapped_pair.values()):  # Checks if any value is None or empty
                print(
                    f"Warning: Skipping pair due to missing data after mapping: {pair} -> {mapped_pair} (raw locode: {city_locode_raw})"
                )
                continue
        ...
        print(
            f"An unexpected error occurred loading {file_path}: {e}. Returning empty list."
        )
        return []
...
def load_models_from_file(file_path: Path) -> list:
    ...
    if not file_path.exists():
        print(
            f"Warning: Model list file not found at {file_path}. Returning empty list."
        )
        return []
    try:
        with open(file_path, "r", encoding="utf-8") as f:
        ...
        if not isinstance(models, list) or not all(
            isinstance(m, dict) and "name" in m for m in models
        ):
            print(
                f"Warning: Model list file {file_path} does not have the expected format (list of dicts with 'name'). Returning empty list."
            )
            return []
...
# --- Prompt Construction ---
def construct_comparison_prompt(
    action_a_data: dict, action_b_data: dict, city_context_data: dict
) -> str:
    """
...
def rank_actions_with_llm(
    ...
    if not all([city_locode, action_a_id, action_b_id]):
        raise ValueError(
            "Missing 'CityLocode', 'ActionA', or 'ActionB' in comparison_input."
        )

    openrouter_headers = (
        initialize_openrouter_client_headers()
    )  # Initialize headers internally
    ...
    all_actions_data = (
        load_all_actions_data()
    )  # This function is defined in prioritizer.py
    ...
    if not action_a_dict:
        raise ValueError(
            f"ActionID '{action_a_id}' (Action A) not found in loaded actions."
        )
    if not action_b_dict:
        raise ValueError(
            f"ActionID '{action_b_id}' (Action B) not found in loaded actions."
        )

    print(
        f"  Comparing Action {action_a_id} vs {action_b_id} for City {city_locode} using model {model_name}"
    )
    ...
    winner_action_id = llm_response_dict.get(
        "actionid"
    )  # Assuming get_openrouter_completion validated presence
    ...
    # The evals expect the return dictionary to have the key 'PreferredAction'
    return {"PreferredAction": winner_action_id}
...
def prioritize_action_pairs(
    ...
    for i, pair_info in enumerate(action_pairs_list):
        ...
        print(
            f"Processing pair {i+1}/{len(action_pairs_list)}: Action A ({action_a_id}) vs Action B ({action_b_id}) for City ({city_locode})"
        )
        ...
        if not all([action_a_id, action_b_id, city_locode]):
            print(
                f"  Skipping pair due to missing IDs or locode in input pair_info: {pair_info}"
            )
            current_result_payload["winner"] = "Error: Missing input data in pair_info"
            current_result_payload["error_detail"] = (
                "One or more of actionA_id, actionB_id, city_locode is missing from the input pair_info."
            )
        ...
        try:
            ...
            elif "LLM returned an invalid ActionID" in str(
                e
            ):  # Checks for invalid ActionID from rank_actions_with_llm
                current_result_payload["winner"] = "Error: Invalid LLM ActionID in JSON"
            elif (
                f"Failed to read city inventory for {city_locode}" in str(e)
                or f"ActionID '{action_a_id}' (Action A) not found" in str(e)
                or f"ActionID '{action_b_id}' (Action B) not found" in str(e)
                or "Failed to load all_actions_data" in str(e)
            ):
...
def save_results(results_data: list, filename: str):
    ...
    try:
        ...
        print(
            f"An unexpected error occurred while trying to save results with 'write_output': {e}"
        )
...
def main():
    ...
    if not OPENROUTER_API_KEY:
        print(
            "Critical Error: OPENROUTER_API_KEY environment variable is not set or not loaded from .env."
        )
        print(
            "Please set this variable in your .env file before running the script. Exiting."
        )
        return
    ...
    if not all_actions_data_check:
        print(
            "Critical Error: Failed to load action data initially, or no actions were found."
        )
        print(
            "Please check the 'data/climate_actions/merged.json' file and 'utils/reading_writing_data.py' paths."
        )
        print("Exiting.")
        return
    ...
    if not current_action_pairs:
        print(
            f"No action pairs loaded from {ACTION_PAIRS_FILE_PATH}. Check the file or path."
        )
